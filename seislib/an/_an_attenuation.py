...
class AmbientNoiseAttenuation:
    ...
    def parameterize(
            ...
            for i, (lat1, lat2, lon1, lon2) in enumerate(mesh[1:], 1):
                if lat1 == lb and lat2 == ub:
                    ...
                    final_indexes.extend(
                        list(np.array(indexes)[np.argsort(mesh[indexes, 2])])
                    )
                    indexes = [i]
                    lb, ub = lat1, lat2
            if indexes:
        ...
        def plot_stations_and_grid(station_coords, grid, map_boundaries):
            ...
            add_earth_features(
                ax, scale=plot_resolution, oceans_color="water", lands_color="land"
            )
        ...
        no_stations, codes = stations_per_pixel(grid, station_codes, station_coords)
        parameterization = {
            "grid": grid.mesh,
        ...
        save_pickle(
            os.path.join(self.savedir, "parameterization.pickle"), parameterization
        )
    ...
    def compute_ffts(self, fs=1, window_length=3600):
        ...
        stations = set(
            [code for codes in self.parameterization["station_codes"] for code in codes]
        )
        for file in self.files:
            ...
            for i, time in enumerate(times):
                ...
                if np.any(~np.isfinite(fft)) or np.all(fft == 0):
                    continue
                store.append(fft)
    ...
    def compute_corr_spectra(
        self, min_no_pairs=6, min_no_days=30, ram_available=9000, ram_split=4
    ):
        """
        ...
        def overlapping_times(recording_stations, times_per_station, window_length):
            common_days = {}
        ...
        def ram_required(stations):
            ...
            for station in stations:
                size = (
                    os.path.getsize(os.path.join(src_ffts, "%s.npy" % station))
                    / 1000000
                )
                required += size
            return required
        ...
        def load_ffts_generator(stations, times, ram_split, verbose):
            ...
            for i in range(ram_split):
                ...
                times_partial = {}
                for sta in stations:
                    itimes = np.flatnonzero((times[sta] >= start) & (times[sta] < end))
                    if itimes.size:
        ...
        def get_psd(
            ...
            psd_dict = compute_psd(
                ffts=ffts, stations=stations, times=times, min_no_pairs=min_no_pairs
            )
        ...
        def compute_egfs(
            ...
            for sta1, sta2 in station_pairs:
                ...
                if os.path.exists(save_file):
                    continue
                if save_memory:
                    ffts = load_ffts(ffts=ffts, stations=(sta1, sta2), times=times)
        ...
        def noisecorr_psd(
            sta1, sta2, ffts, times, psd_dict, frequency, window_length, min_no_days
        ):
            ...
            notnan = [i for i, c in enumerate(corr_normalized) if ~np.any(np.isnan(c))]
        ...
        for ipixel in range(self.parameterization["grid"].shape[0]):
            ...
            common_days = overlapping_times(
                recording_stations=stations,
    ...
    def prepare_inversion(
        ...
        def get_envelope(
            x, y, freq, smoothed=False, smoothing_poly=None, smoothing_window=None
        ):
            """
            ...
            spline = CubicSpline(
                x[peaks], y[peaks], bc_type="natural", extrapolate=False
            )
            envelope = spline(freq)
            if smoothed:
                notnan = np.flatnonzero(~np.isnan(envelope))
                envelope[notnan] = savgol_filter(
                    envelope[notnan], smoothing_window, smoothing_poly
                )
        ...
        def get_data_dict(station_pair, tmp_dict, freq, freq_corr, **kwargs):
            c_w = interp1d(
                tmp_dict["freq"], tmp_dict["c_w"], bounds_error=False, fill_value=np.nan
            )(freq)
            try:
                smoothed_egf = velocity_filter(
                    freq_corr,
                    tmp_dict["egf"],
                    tmp_dict["r"] / 1000,
                    p=0.1,
                ...
                envelope_egf = get_envelope(
                    x=freq_corr, y=smoothed_egf.real, freq=freq, **kwargs
                )
                envelope_j0 = get_envelope(
                    x=tmp_dict["freq"], y=tmp_dict["j0"], freq=freq, **kwargs
                )
            except ValueError:
                return

            smoothed_egf = np.interp(
                freq, freq_corr, smoothed_egf, left=np.nan, right=np.nan
            )
            bessel = np.interp(
                freq, tmp_dict["freq"], tmp_dict["j0"], left=np.nan, right=np.nan
            )
            return {
                "egf": smoothed_egf,
                "j0": bessel,
        ...
        pixels = sorted(
            [i for i in os.listdir(src_corr) if "pixel" in i],
        ...
        for pixel in pixels:
            ...
            tmp_data = load_tmp_data(
                corr_dir=os.path.join(src_corr, pixel),
                src_velocity=src_velocity,
                velocity_files_dict=velocity_files_dict,
    ...
    def inversion(self, alphamin=5e-7, alphamax=5e-4, nalpha=350, min_no_pairs=6):
            ...
            for pair in data:
                ...
                for ialpha, alpha in enumerate(alphas):
                    model, cost = cost_function(
                        j0_env=j0_env, egf_env=egf_env, alpha=alpha, r=r
                    )
        ...
        pixels = sorted(
            [i for i in os.listdir(src_dataset) if "pixel" in i],
    ...
    def get_attenuation_map(self, period, cell_size, min_overlapping_pixels=2):
        ...
        def get_related_pixels(mesh, lon1, lon2, lat1, lat2):
            ...
            idx = np.flatnonzero(
                (((lon1 > lons1) & (lon1 < lons2) & (lat1 > lats1) & (lat1 < lats2)))
                | (((lon2 > lons1) & (lon2 < lons2) & (lat1 > lats1) & (lat1 < lats2)))
                | (((lon1 > lons1) & (lon1 < lons2) & (lat2 > lats1) & (lat2 < lats2)))
                | (((lon2 > lons1) & (lon2 < lons2) & (lat2 > lats1) & (lat2 < lats2)))
            )
            return idx
        ...
        alphas, no_measurements = interpolate_results(
            freq=freq, results=results, period=period
        )

        mesh = mesh[sorted(results)]
        latmin, lonmin = np.min(mesh, axis=0)[::2]
        latmax, lonmax = np.max(mesh, axis=0)[1::2]
        new_mesh = EqualAreaGrid(
            cell_size=cell_size,
            lonmin=lonmin,
            lonmax=lonmax,
            latmin=latmin,
        ...
        for i, (lat1, lat2, lon1, lon2) in enumerate(new_mesh):
            related_pixels = get_related_pixels(
                mesh, lon1=lon1, lon2=lon2, lat1=lat1, lat2=lat2
            )
    ...
    def plot_azimuthal_coverage(self, ipixel, ax=None, bins=None, show=True, **kwargs):
        ...
        **kwargs : dict, optional
        ...
        """
        data = load_pickle(
            os.path.join(self.savedir, "inversion_dataset", "pixel%s.pickle" % ipixel)
        )
        pairs = [i.split("__") for i in data]
        coords = np.array(
            [[*self.stations[pair[0]], *self.stations[pair[1]]] for pair in pairs]
        )
    ...
    def plot_cost(
        ...
        **kwargs : dict, optional
        ...
        bbox_props = dict(
            boxstyle="round", fc="w", ec="none", lw=1.5, pad=0.1, alpha=0.85
        )

        if ax is None:
            fig, ax = plt.subplots()
        img = ax.pcolormesh(
            frequency, alpha, cost_normalized, norm=norm, cmap=cmap, **kwargs
        )
