...
def get_zero_crossings(freq, xcorr, dist, freqmin=0, freqmax=1, cmin=1, cmax=5, 
    ...
    pos_j_crossings = j_zeros[1::2]
    neg_j_crossings = j_zeros[0::2]
    ...
    freq_pos_cross, vel_pos_cross = [], []
    freq_neg_cross, vel_neg_cross = [], []
    for pos_cross in pos_crossings:
        vel = pos_cross * 2 * np.pi * dist / pos_j_crossings
        vel = vel[(vel>cmin) & (vel<cmax)]        
        freq_pos_cross.extend(np.ones(vel.size) * pos_cross)
        vel_pos_cross.extend(vel[::-1])        
    for ncross in neg_crossings:
        vel = ncross * 2 * np.pi * dist / neg_j_crossings
        vel = vel[(vel>cmin) & (vel<cmax)]
        freq_neg_cross.extend(np.ones(vel.size) * ncross)
        vel_neg_cross.extend(vel[::-1])
    
    freq_crossings = np.concatenate((freq_pos_cross, freq_neg_cross))
    vel_crossings = np.concatenate((vel_pos_cross, vel_neg_cross))
    return np.column_stack((freq_crossings, vel_crossings))


def extract_dispcurve(frequencies, corr_spectrum, interstation_distance, ref_curve,
                      freqmin=0.0, freqmax=1.0, cmin=1.0, cmax=5.0, filt_width=7, 
                      filt_height=0.8, x_step=None, pick_threshold=2.,
                      horizontal_polarization=False, plotting=False, savefig=None):
    """
    ...
    def get_slope(picks, freq, x_step, reference_curve_func):
        """
    ...
    def get_zero_crossing_slopes(crossings, freq, reference_velocity, slope,
                                 width, reference_curve_func,
    ...
    def get_kernel(X, Y, freq, vel, slope, interstation_distance, fstep,
                   filt_width, filt_height, return_poly_coords=False,
    ...
    def update_density_field(X, Y, density, crossings, fstep, filt_width, 
                             filt_height, interstation_distance,
    ...
    def pick_velocity(picks, frequency, densities, slope, x_step, minvel, maxvel,
        ...
        reference_curve_func : scipy.interpolate.interp1d
...
#                ellipse_paths += ell_paths
        ...
        slope = get_slope(picks, freq_pick, x_step, reference_curve_func)
    ...
    if plotting:  
        ...
        for cross in crossings:
            ax.plot([cross[0]-length, cross[0]+length], 
                    [cross[1]+cross[2]*-length, cross[1]+cross[2]*length],
                    'y-')
        ax.plot(zero_crossings[:,0], zero_crossings[:,1], 'wo',
                markeredgecolor='black', ms=2, linewidth=0.3, label='zero crossings')
        ax.plot(ref_curve[:,0], ref_curve[:,1], linewidth=2, color='lightblue',
...
#            ax.plot(vertices[:,0], vertices[:,1], color='white', linewidth=0.5)
        ...
        ax.annotate("Distance: %d km"%interstation_distance, 
