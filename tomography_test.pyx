...
@boundscheck(False)
@wraparound(False) 
def _refine_parameterization_w_tests(double[:, ::1] mesh, double[:, ::1] coeff_matrix, double[:, ::1] data_coords, int hitcounts=100, double[:] region_to_refine=None):
    ...
    # @amine : Data I added
    #print("Hello from my local _refine_parametrization_w_tests!")
    size_subcell = 4
    cdef double[:, ::1] tempmesh = np.zeros((size_subcell, 4), dtype=np.double)
    cdef double[:, ::1] tempcoeff_matrix = np.zeros((data_size, size_subcell), dtype=np.double)
    do_we_split_bool = np.array([False for i_pixel in range(mesh_size)], dtype = bool) # boolean to save the decision to split or not for each cell
    ...
    for i_pixel in range(mesh_size):
        ...
        if rays_count[i_pixel]>=hitcounts and inside_region:
            ...
            #print(f"Do we split cell : {np.asarray(mesh[i_pixel, :])} ?")
            #print(f"Ray count : {rays_count[i_pixel]}")
            #print()

            # But first we check whether there are enough rays in each subcell
            # We prepare the subcells to test
            split_pixel(tempmesh, 0, parallel1, parallel2, meridian1, meridian2)
            #print("Temporary mesh:")
            #print(np.asarray(tempmesh, dtype = np.double))
            #print()

            mesh_latmax = np.max([parallel1, parallel2])
            mesh_lonmax = np.max([meridian1, meridian2])
            # We keep the rays that hit the cell
            temp_data_coords = []
            for i_data in range(data_size):
                if coeff_matrix[i_data, i_pixel] != 0:
                    temp_data_coords.append(data_coords[i_data, :])
            temp_data_coords = np.asarray(temp_data_coords)
            #print(f"Size of temp_data_coords : {temp_data_coords.shape[0]}/{np.asarray(data_coords, dtype = np.double).shape[0]}")
            #print("Temporary data coordinates : (10 elements)")
            #print(np.asarray(temp_data_coords, dtype = np.double)[:10])
            #print()

            # I printed this to compare the provided coeff_matrix with the temporary one I made
            #print("Coefficient matrix : (10 elements)")
            #print(np.asarray(coeff_matrix, dtype = np.double)[:10])

            temp_data_coords = np.radians(temp_data_coords)
            tempmesh = np.radians(tempmesh)
            mesh_latmax = np.radians(mesh_latmax)
            mesh_lonmax = np.radians(mesh_lonmax)
            tempcoeff_matrix = _compile_coefficients(temp_data_coords, tempmesh, mesh_latmax, mesh_lonmax)
            #print("Temporary coefficient matrix : (10 elements)")
            #print(np.asarray(tempcoeff_matrix, dtype = np.double)[:10])
            temp_rays_count = _raypaths_per_pixel(tempcoeff_matrix)
            #print("Ray count:")
            #print( np.asarray(temp_rays_count, dtype = np.int32) )
            #print()

            if np.all(np.asarray(temp_rays_count, dtype = np.int32) >= hitcounts):
                #print("We do split")
                do_we_split_bool[i_pixel] = True
                # Remove one pixel and add 4 -- the pixels itself divided by four
                newmesh_size += 3
            #else:
                #print("We do not split; subcells are underparametrized")
            #print("---------------------------------------")
            #print()
            #print()
    ...
    for i_pixel in range(mesh_size):
        if do_we_split_bool[i_pixel] == True :
            parallel1 = mesh[i_pixel, 0]
            parallel2 = mesh[i_pixel, 1]
            meridian1 = mesh[i_pixel, 2]
            meridian2 = mesh[i_pixel, 3]    
